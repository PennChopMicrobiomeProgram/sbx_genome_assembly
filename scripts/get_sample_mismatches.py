...
all_fasta_sequences = {fa.description: fa.seq for fa in SeqIO.parse(snakemake.input.all, 'fasta')}
query_fasta_sequences = {fa.description: fa.seq for fa in SeqIO.parse(snakemake.input.sample, 'fasta')}
...
with open(snakemake.output[0], 'w') as out:
    ...
    for keys in query_genes:
        ...
        subj_names = [names for names,gene_name in all_genes.items() if gene_name == query_genes[keys]]
        for subj in subj_names:
            subj_seq = all_seqs[subj]
            mismatches = format_mismatch(*pairwise2.align.globalms(query_seq, subj_seq, 5, -4, -10, -4, penalize_extend_when_opening=True, penalize_end_gaps=False, one_alignment_only=True)[0])
