...
sunbeam run --configfile ./sunbeam_config.yml --use-conda all_SPARCQ -j 50 -w90 -k --keep-going --ignore-incomplete --cluster-config ./cluster.json --notemp -p --nolock --verbose -c 'sbatch --no-requeue --export=ALL --mem={cluster.memcpu} -n {threads} -t 10-0 -J {cluster.name} --output=slurm_%x_%j.out'
